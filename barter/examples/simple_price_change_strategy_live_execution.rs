...
use barter_execution::{
    ...
    client::binance::BinanceExecutionConfig,
    order::{
...
// 策略参数
const BUY_THRESHOLD_PERCENT: Decimal = dec!(2.0); // 下跌2%买入
const SELL_THRESHOLD_PERCENT: Decimal = dec!(5.0); // 上涨5%卖出
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 加载配置（只加载 instruments，不加载 executions）
    let SystemConfig {
        instruments,
        executions: _, // 实盘交易不使用配置文件中的 executions
    } = load_config()?;
    ...
    // ====================================================================
    //
    // 注意：目前 barter-execution 中的币安实盘客户端尚未实现。
    // 要实现实盘交易，需要：
    //
    // 1. 实现币安实盘执行客户端（BinanceExecutionClient）：
    //    - 在 barter-execution/src/client/binance/mod.rs 中实现 ExecutionClient trait
    //    - 需要实现以下方法：
    //      * account_snapshot() - 获取账户快照
    //      * account_stream() - 获取账户事件流（WebSocket）
    //      * open_order() - 下单
    //      * cancel_order() - 撤单
    //    - 需要处理币安 API 认证、签名、错误处理等
    //
    // 2. 创建币安实盘配置结构：
    //    ```rust
    //    #[derive(Debug, Clone)]
    ...
    //        pub base_url: String, // 例如 "https://api.binance.com"
    //        pub testnet: bool,    // 是否使用测试网
    //    }
    //    ```
    //
    // 3. 修改 SystemBuilder 以支持实盘执行配置：
    //    - 在 barter/src/system/config.rs 中添加 ExecutionConfig::Live 变体
    //    - 在 barter/src/system/builder.rs 中处理 Live 配置
    //
    // 4. 使用实盘客户端：
    //    ```rust
    //    use barter_execution::client::binance::BinanceExecutionClient;
    //
    //    let binance_config = BinanceExecutionConfig {
    ...
    //    };
    //
    //    let execution = ExecutionBuilder::new(&instruments)
    //        .add_live::<BinanceExecutionClient>(binance_config, Duration::from_secs(30))?
    //        .build();
    //    ```
    //
    // 由于币安客户端尚未实现，这里暂时使用空配置列表。
    // 实际使用时，需要先实现币安客户端，然后取消下面的注释并配置。
    // ====================================================================
    ...
    // let execution = ExecutionBuilder::new(&instruments)
    //     .add_live::<BinanceExecutionClient>(binance_config.clone(), Duration::from_secs(30))?
    //     .build();

    // 构建系统参数（使用空的 executions，因为实盘客户端尚未实现）
    let args = SystemArgs::new(
        &instruments,
        vec![ExecutionConfig::Live(binance_config.clone())],
        LiveClock,
        PriceChangeStrategy::default(),
        PositionLimitRiskManager::default(),
        market_stream,
